...
/* Pass in one of FATAL, ERROR, WARN,  INFO and DEBUG to modify your log level;
    ...
    protected File getUploadDir() {
        if (uploadDir.length() == 0) {
            uploadDir = RuntimeAccess.getInstance().getSession().getServletContext().getRealPath(getRelativUploadDir());
        }
    ...
    public FileUploadResponse uploadFile(MultipartFile file) throws IOException {
        ...
        try { /* Find our upload directory, make sure it exists */
            ...
            ret.setPath(getRelativUploadDir().concat("/").concat(outputFile.getName()));
            ret.setError("");
            ret.setWidth("");
            ret.setHeight("");
        } catch (Exception e) {
